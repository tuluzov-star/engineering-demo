...
## Portfolio screenshot proof

README screenshots are generated by Playwright against the **production standalone Docker target**, after dependency-aware readiness succeeds. The committed captures cover:

- desktop overview
- live Store API checkout state
- 390 px mobile layout

The normal CI run also uploads the full production screenshot set as a temporary artifact for visual review.

## Portfolio video proof

The short demo video is generated from the **production standalone Docker target**, not from a development server or a manually edited screen recording.

The automated recording:

- launches Chromium at 1280×720;
- shows live runtime metadata and architecture;
- adds a product through the Next.js BFF;
- updates cart quantity;
- opens Store API checkout;
- creates a real WooCommerce order with HPOS enabled;
- shows the resulting order ID/status;
- uses short technical overlays injected only for the recording.

Playwright records WebM; FFmpeg converts it to H.264 MP4 with fast-start metadata. The verified recording is about 17.72 seconds and the committed MP4 is about 519 KB. CI also uploads MP4/WebM artifacts.
